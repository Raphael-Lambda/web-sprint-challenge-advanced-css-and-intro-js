...
function getArtistByIndex(array1, index) {
  return `the artist at index ${array1[index].id} is ${array1[index].name}`
}  
...
function get20s(array1){
  let newarr = array1.filter(x => Number(x.years.split(' - ')[0]) >= 1900 && Number(x.years.split(' - ')[1]) <= 2000);
  return newarr.map(x => x.name)
}
...
function removeArtist(array1, index){
...
function lotsOfArt(array1){
  let alot = array1.filter(x => x.paintings > 100);
  return alot.map(x => x.name);
}
